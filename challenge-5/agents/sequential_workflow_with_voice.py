"""
Enhanced Sequential Workflow with Voice Call Integration

This workflow extends the 4-executor architecture with a 5th executor for voice calling:
Customer Data → Risk Analyzer → (Compliance Report + Fraud Alert + Voice Call)

The Voice Call executor runs in parallel with Compliance Report and Fraud Alert executors,
enabling real-time customer notification and verification of potentially fraudulent transactions.
"""
...
"""
                ...
                # Send data to all three parallel executors (compliance report, fraud alert, AND voice call)
...
@executor
async def voice_call_executor(
    risk_response: RiskAnalysisResponse,
    ctx: WorkflowContext[Never, VoiceCallResponse]
) -> None:
    """Voice Call Executor that initiates outbound calls to verify fraudulent transactions."""

    try:
        # Parse risk score to determine if call is needed
        parsed_analysis = parse_risk_analysis_result(risk_response.risk_analysis)
        risk_score = parsed_analysis.get("parsed_elements", {}).get("risk_score", 0)
        
        # Only initiate calls for high-risk transactions (score >= 75)
        if risk_score < 75:
            result = VoiceCallResponse(
                call_id="NO_CALL_NEEDED",
                call_status="SKIPPED",
                ...
                transaction_id=risk_response.transaction_id,
                status="SUCCESS",
                error_message=f"Risk score {risk_score} below threshold for voice verification"
            )
            await ctx.yield_output(result)
            return
        
        # Extract customer information from risk response
        # Parse customer data to get phone number and name
        customer_data_lines = risk_response.customer_data.split('\n')
        phone_number = ""
        customer_name = ""
        ...
        for line in customer_data_lines:
            ...
            if "Name:" in line:
                customer_name = line.split("Name:")[1].strip()
        ...
        if not phone_number or phone_number == "N/A":
            result = VoiceCallResponse(
                ...
                verification_status="NO_CONTACT_INFO",
                customer_response="",
                recommended_action="INVESTIGATE",
                transaction_id=risk_response.transaction_id,
                status="ERROR",
                error_message="Customer phone number not available"
            )
            await ctx.yield_output(result)
            return
        
        # Get transaction details for the call
        transaction_id = risk_response.transaction_id
        transaction_data = get_transaction_data(transaction_id)
        
        if "error" in transaction_data:
            result = VoiceCallResponse(
                call_id="NO_TRANSACTION",
                call_status="FAILED",
                call_initiated=False,
                ...
                status="ERROR",
                error_message=f"Could not retrieve transaction data: {transaction_data.get('error')}"
            )
            await ctx.yield_output(result)
            return
        ...
        if not acs_connection_string:
            result = VoiceCallResponse(
                call_id="ACS_NOT_CONFIGURED",
                call_status="SKIPPED",
                call_initiated=False,
                target_phone_number=phone_number,
                customer_name=customer_name,
                verification_status="ACS_NOT_AVAILABLE",
                customer_response="",
                recommended_action="INVESTIGATE",
                transaction_id=transaction_id,
                status="SUCCESS",
                error_message="ACS not configured - voice calling disabled. Set ACS_CONNECTION_STRING and ACS_PHONE_NUMBER in .env to enable."
            )
            await ctx.yield_output(result)
            return
        
        # Initiate voice call using ACS
        call_result = create_fraud_verification_call_sync(
            target_phone_number=phone_number,
            customer_name=customer_name,
            transaction_id=transaction_id,
        ...
        if call_result.get("success"):
            result = VoiceCallResponse(
                ...
                customer_response="Call initiated - awaiting customer response",
                recommended_action="PENDING_VERIFICATION",
                transaction_id=transaction_id,
                status="SUCCESS",
                error_message=""
            )
        ...
        error_result = VoiceCallResponse(
            call_id="ERROR",
            call_status="ERROR",
            call_initiated=False,
            target_phone_number="",
            ...
            transaction_id=risk_response.transaction_id if risk_response else "Unknown",
            status="ERROR",
            error_message=f"Error in voice call processing: {str(e)}"
        )
        await ctx.yield_output(error_result)


async def run_fraud_detection_workflow():
    """Execute the enhanced fraud detection workflow with voice calling."""

    # Build workflow with five executors - parallel execution for compliance, fraud alert, and voice call
    workflow = (
        WorkflowBuilder()
        .set_start_executor(customer_data_executor)
        .add_edge(customer_data_executor, risk_analyzer_executor)
        # Parallel path 1: Compliance Report
        .add_edge(risk_analyzer_executor, compliance_report_executor)
        # Parallel path 2: Fraud Alert
        .add_edge(risk_analyzer_executor, fraud_alert_executor)
        # Parallel path 3: Voice Call (NEW!)
        .add_edge(risk_analyzer_executor, voice_call_executor)
        .build()
    )

    # Create request
    request = AnalysisRequest(
        message="Comprehensive fraud analysis with voice verification",
        transaction_id="TX1015"  # Russian transaction for testing - high risk
    )

    # Execute workflow with streaming
    compliance_output = None
    fraud_alert_output = None
    voice_call_output = None

    print("🔄 Executing 5-Executor Fraud Detection Workflow with Voice Calling...")

    async for event in workflow.run_stream(request):
        # Capture outputs from all three parallel executors
        if isinstance(event, WorkflowOutputEvent):
            ...
            elif isinstance(event.data, VoiceCallResponse):
                voice_call_output = event.data

    return compliance_output, fraud_alert_output, voice_call_output


async def main():
    """Main function to run the enhanced fraud detection workflow."""
    try:
        compliance_result, fraud_alert_result, voice_call_result = await run_fraud_detection_workflow()
        ...
        # Display Voice Call results (NEW!)
        if voice_call_result and isinstance(voice_call_result, VoiceCallResponse):
            print(f"\n📞 VOICE CALL EXECUTOR (NEW):")
            print(f"   Status: {voice_call_result.status}")
            print(f"   Transaction ID: {voice_call_result.transaction_id}")
            print(f"   Call ID: {voice_call_result.call_id}")
            print(
                f"   Call Initiated: {'✅ YES' if voice_call_result.call_initiated else '❌ NO'}")
            print(f"   Call Status: {voice_call_result.call_status}")
            print(f"   Target Phone: {voice_call_result.target_phone_number}")
            print(f"   Customer Name: {voice_call_result.customer_name}")
            print(f"   Verification Status: {voice_call_result.verification_status}")
            print(f"   Recommended Action: {voice_call_result.recommended_action}")
            if voice_call_result.customer_response:
                print(f"   Customer Response: {voice_call_result.customer_response}")
            if voice_call_result.error_message:
                print(f"   ⚠️  Error: {voice_call_result.error_message}")
        else:
            print(f"\n📞 VOICE CALL EXECUTOR: ❌ FAILED")

        print(f"\n✅ 5-EXECUTOR PARALLEL WORKFLOW COMPLETED")
        print(f"   Architecture: Customer Data → Risk Analyzer → (Compliance Report + Fraud Alert + Voice Call)")

        return compliance_result, fraud_alert_result, voice_call_result
...
if __name__ == "__main__":
    compliance, fraud_alert, voice_call = asyncio.run(main())
